...
{
    public static void ThrowIfFails(BOOL result)
    {
        if (!result) throw new Win32Exception(Marshal.GetLastWin32Error());
    }

    public static void ThrowIfFails(HRESULT hResult)
    {
        if (hResult.Value != (int)ResultCode.S_OK) throw new Win32Exception(hResult.Value);
    }
}
